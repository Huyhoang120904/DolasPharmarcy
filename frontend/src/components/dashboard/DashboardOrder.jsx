...
const DashboardOrder = () => {
  ...
  const statusColors = {
    PENDING: { bg: "bg-yellow-400", text: "!text-white" },
    SHIPPING: { bg: "bg-purple-400", text: "!text-white" },
    COMPLETED: { bg: "bg-green-400", text: "!text-white" },
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      const response = await OrderService.getOrderByPage();
  ...
  const StatusBadge = ({ status }) => {
    ...
    const displayStatus =
      statusMapping[normalizedStatus] || statusMapping[status] || status;
    return (
      <span
        className={`inline-flex items-center px-3 py-1.5 rounded-full text-lg font-medium ${statusStyle.bg} ${statusStyle.text}`}
      >
              ...
              {key !== "all" && (
                <span
                  className={`w-1.5 h-1.5 rounded-full mr-2 ${
                    statusColors[key]?.bg || "bg-gray-400"
                  }`}
                ></span>
              )}
              {value}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              {statuses.map((status) => (
                <div
                  key={status.value}
                  className={`border rounded-md p-5 cursor-pointer flex items-center ${
                  ...
                  <div className="flex items-center flex-1">
                    <span
                      className={`w-2 h-2 rounded-full mr-3 ${
                        statusColors[status.value]?.bg || "bg-gray-400"
                      }`}
                    ></span>
        ...
        <div className="bg-white p-8 rounded-lg shadow-lg w-11/12 max-w-2xl">
          ...
          <div className="mb-8">
            <p className="text-lg text-gray-700">
              Bạn có chắc chắn muốn xóa đơn hàng #{selectedOrderIdToDelete}?
            </p>
          ...
          <div className="space-y-5 mb-8">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setExportOptions({ ...exportOptions, status: e.target.value })
                }
              >
                {Object.entries(statusMapping).map(([key, value]) => (
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-base text-gray-500">
                        {selectedOrder.userId || "Khách vãng lai"}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <div className="text-lg text-gray-600">
                        {selectedOrder.address}, {selectedOrder.ward},{" "}
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-5 py-5 text-left text-base font-medium text-gray-500 uppercase tracking-wider">
                          Sản phẩm
                        </th>
                        <th className="px-5 py-5 text-left text-base font-medium text-gray-500 uppercase tracking-wider">
                          Loại
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {selectedOrder.items?.map((item) => (
                        <tr key={item.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-5 py-5 text-lg text-gray-600">
                            {item.variant?.name || "-"}
                          </td>
                          <td className="px-5 py-5">
                            <div>
                          ...
                          <td className="px-5 py-5 text-lg text-gray-600 font-medium">
                            {item.quantity}
                          </td>
                          <td className="px-5 py-5 text-lg font-medium text-blue-600">
                    ...
                    <tfoot>
                      <tr className="bg-gray-50 font-medium">
                        <td
                          colSpan="4"
                          className="px-5 py-5 text-right text-lg"
                        >
                          Tổng cộng:
                        </td>
                        <td className="px-5 py-5 text-blue-600 font-bold text-lg">
                          {formatCurrency(selectedOrder.total)}
                        </td>
                      </tr>
                    </tfoot>
      ...
      <div className="w-full max-w-full mx-auto p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-5 mb-8">
          {Object.entries(statusMapping).map(([key, value]) => {
            ...
            const statusStyle = statusColors[key] || {
              bg: "bg-gray-400",
              text: "text-gray-800",
            };
            return (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <div
                    className={`w-3 h-3 rounded-full ${statusStyle.bg}`}
                  ></div>
                </div>
              </div>
            );
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-6 text-left text-lg font-medium text-gray-500 uppercase tracking-wider">
                    Mã đơn
                  </th>
                  <th className="px-6 py-6 text-left text-lg font-medium text-gray-500 uppercase tracking-wider">
                    Khách hàng
                  ...
                  <th className="px-6 py-6 text-right text-lg font-medium text-gray-500 uppercase tracking-wider">
                    Thao tác
                  </th>
                </tr>
              </thead>
                ...
                {!loading && filteredOrders.length > 0 ? (
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      <td className="px-6 py-6 text-blue-600 font-medium text-xl">
                        #{order.id}
                      </td>
                      ...
                      <td className="px-6 py-6 text-xl text-gray-600">
                        {formatDate(order.createAt)}
                      </td>
                      <td className="px-6 py-6 text-xl text-gray-600">
                        {formatDate(order.receiveDate)}
                      </td>
                      <td className="px-6 py-6 text-xl font-medium text-blue-600">
          ...
          {filteredOrders.length > 0 && (
            <div className="px-6 py-5 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
              <div className="text-lg text-gray-500">
                Hiển thị {filteredOrders.length} trong số {orders.length} đơn
                hàng
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="px-5 py-2 bg-blue-100 text-blue-600 font-medium rounded-md text-lg">
                  1
                </span>
                <button className="px-5 py-2 border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-lg cursor-pointer">
                  Sau
